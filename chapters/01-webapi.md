...
**何をしているか：**
```markdown
この処理では、入力されたアーティスト名をもとに iTunes Search API にアクセスし、
返ってきたJSONデータを SearchResponse 型に変換して、
検索結果を songs に保存しています。
また、通信中は isLoading を使って読み込み中の表示も行っています。
```

**なぜこう書くのか：**
```markdown
API通信では、検索文字をURLで使える形に変換し、正しいURLを作って、
ネットワーク経由でデータを受け取る必要があります。
また、返ってきたJSONをSwiftで扱いやすい形に変換し、
通信失敗時にも対応できるように do-catch を使っています。
さらに、通信中であることをユーザーにわかりやすくするために isLoading も使っています。
````

**もしこう書かなかったら：**
```markdown
検索文字が正しくURLに入らず通信に失敗したり、
APIからデータを取得できなかったり、
JSONをSwiftのデータに変換できなくなったりします。また、エラー時の処理ができずアプリが不安定になったり、
通信中かどうかが画面からわからなくなったりします。
```
