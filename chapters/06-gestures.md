...
また、複数のジェスチャーを同時に組み合わせる方法も学ぶことができ、interactive なアプリ開発の基礎を身につけられます。
...
1. **質問：**　@State はどのような役割を持ち、画面はどのように更新されるのですか？
   **得られた理解：**　@State は画面の状態を保持するためのもので、値が変更されると SwiftUI が自動で画面を再描画することが分かった。ジェスチャーで offset や scale が変化すると、画面もリアルタイムで更新されることを理解できた。

2. **質問：**　　ドラッグ処理で lastOffset.width + value.translation.width と書く理由は何ですか？
   **得られた理解：**　　value.translation は現在のドラッグ開始位置からの移動量だけを表すことが分かった。そのため、前回の位置を lastOffset に保存し、移動量を加算することで、ドラッグを繰り返しても正しい位置に移動できることを理解した。

3. **質問：**　　DispatchQueue.main.asyncAfter や withAnimation(.spring) はどのような役割がありますか？
   **得られた理解：**　　DispatchQueue.main.asyncAfter は一定時間後に処理を実行するために使われ、今回は1秒後に元の状態へ戻していることが分かった。また、withAnimation(.spring) を使うことで、画面の変化を自然で滑らかなアニメーションにできることを理解した。
